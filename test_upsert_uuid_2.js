...
(async () => {
  ...
  const validUUID = `00000000-0000-4000-8000-${hexTs}`;

  const testProf = {
    id: validUUID,
    full_name: 'Test Valid Hex UUID User',
    email: `validhex_${Date.now()}@example.com`,
    gender: 'Boys',
    bio: 'Testing profile upsert with valid hex UUID format',
    city_id: 'ranchi',
  ...
  if (error) {
    ...
    console.log('UPSERT SUCCESS! ID:', validUUID);
  }
})();
