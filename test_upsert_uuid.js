...
(async () => {
  const ts = String(Date.now()).padStart(12, '0');
  const validUUID = `00000000-0000-4000-8000-${ts}`;

  const testProf = {
    id: validUUID,
    full_name: 'Test Valid UUID User',
    email: `valid_${Date.now()}@example.com`,
    gender: 'Boys',
    bio: 'Testing profile upsert with valid UUID format',
    city_id: 'ranchi',
