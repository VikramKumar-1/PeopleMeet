...
import { Home, Sparkles, MapPin, CheckCircle, MessageSquare, Phone, ShieldCheck, Search, Navigation } from 'lucide-react';
import { FlatListing, FlatCategory, BhkType, CityHub } from '@/types';
...
export default function FlatView({
  ...
  const [filterBhk, setFilterBhk] = useState<BhkType | 'All'>('All');
  const [filterCategory, setFilterCategory] = useState<FlatCategory | 'All'>('All');
  const [searchQuery, setSearchQuery] = useState<string>('');
  const [onlyRecommended2Km, setOnlyRecommended2Km] = useState<boolean>(false);

  const filteredFlats = useMemo(() => {
    return flats.filter((flat) => {
      // Show all flats across whole city limits
      ...
      // Category filter
      if (filterCategory !== 'All' && flat.category !== filterCategory) return false;

      // Recommended nearest within 2 km (2000m)
      if (onlyRecommended2Km && flat.distanceMeter > 2000) return false;

      // Keyword search (matches title, hub, bhk, furnishing, or category)
      if (searchQuery.trim() !== '') {
        const query = searchQuery.toLowerCase();
        const matchTitle = flat.title.toLowerCase().includes(query);
        const matchHub = flat.hub.toLowerCase().includes(query);
        const matchBhk = flat.bhk.toLowerCase().includes(query);
        const matchCat = flat.category.toLowerCase().includes(query);
        const matchFurn = flat.furnishing.toLowerCase().includes(query);
        if (!matchTitle && !matchHub && !matchBhk && !matchCat && !matchFurn) return false;
      }

      return true;
    });
  }, [flats, currentCity.id, filterBhk, filterCategory, onlyRecommended2Km, searchQuery]);

  const bhkChips: { label: string; value: BhkType | 'All' }[] = [
    { label: 'All BHKs', value: 'All' },
    { label: '1 BHK / Studio', value: '1 BHK' },
    { label: '2 BHK', value: '2 BHK' },
    { label: '3 BHK Floor', value: '3 BHK' },
  ];

  const catChips: { label: string; value: FlatCategory | 'All' }[] = [
    { label: 'All Categories', value: 'All' },
    { label: 'Independent', value: 'Independent' },
    { label: 'Bachelors Allowed', value: 'Bachelor Allowed' },
    ...
    <div className="space-y-4 sm:space-y-6 pb-24 sm:pb-28">
      ...
      <div className="glass-card rounded-3xl p-4 sm:p-6 border border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xl">
        <div>
          <div className="flex items-center gap-1.5 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-cyan-400">
            ...
            <span>Whole City Flat Directory</span>
          </div>
          <h2 className="text-lg sm:text-2xl font-extrabold text-white tracking-tight mt-0.5">
            Flats & Studio Rooms across {currentCity.name}
          </h2>
          <p className="text-[11px] sm:text-sm text-slate-400 mt-0.5">
            Showing all apartments across city. Search specific keyword or hub below.
          </p>
        </div>
        ...
        >
          ...
          <span>⚡ Feature Your Flat (₹500/mo)</span>
        </button>
      </div>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder={`Search keyword across ${currentCity.name} (e.g., '2 BHK Boring Road', 'Studio Lalpur')...`}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
          {/* Swipable Horizontal Category Chips */}
          <div className="space-y-2 flex-1">
            <div className="flex overflow-x-auto no-scrollbar items-center gap-2 py-1 -mx-1 px-1 touch-pan-x">
              <span className="text-[11px] font-semibold text-slate-400 shrink-0 mr-1">Size:</span>
              {bhkChips.map((chip) => (
                <button
                  key={chip.value}
                  onClick={() => setFilterBhk(chip.value)}
                  className={`px-3.5 py-1.5 rounded-xl font-bold text-xs transition-all shrink-0 active:scale-95 ${
                    filterBhk === chip.value
                      ? 'bg-cyan-400 text-slate-950 shadow-md scale-105 font-black'
          ...
          <button
            ...
            className={`self-start sm:self-center px-3.5 py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 border transition-all active:scale-95 shrink-0 ${
              onlyRecommended2Km
          ...
          >
            <div>
              <div className="relative h-44 sm:h-56 w-full overflow-hidden bg-slate-950">
                ...
                {flat.isPrime && (
                  <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-gradient-to-r from-amber-500 to-orange-600 text-black font-extrabold text-[10px] sm:text-[11px] uppercase tracking-wider flex items-center gap-1 shadow-lg border border-amber-300/40">
                    <Sparkles className="h-3 w-3" /> ⭐ Verified Prime Flat
                  </div>
                )}
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white">
                  <span className="px-2.5 py-1 rounded-lg bg-slate-900/90 backdrop-blur-md text-xs font-bold border border-slate-800 text-cyan-400">
                    {flat.bhk} ({flat.category}) • {flat.hub}
                  </span>
                  <span className="px-2.5 py-1 rounded-lg bg-slate-900/90 backdrop-blur-md text-xs font-semibold border border-slate-800 flex items-center gap-1 text-slate-200">
                    <MapPin className="h-3 w-3 text-cyan-400" /> {flat.distanceMeter}m away
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-5 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5">
                      Hub: <strong className="text-cyan-400">{flat.hub}</strong> • Furnishing: <strong className="text-slate-200">{flat.furnishing}</strong>
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-base sm:text-xl font-black text-amber-400">
                      ₹{flat.rentPerMonth.toLocaleString()}
                    </div>
                ...
                <div className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800/80 text-[11px] sm:text-xs text-slate-300 flex items-center justify-between">
                  <span>Security Deposit: <strong className="text-white">₹{flat.securityDeposit.toLocaleString()}</strong></span>
            ...
            <div className="p-3 sm:p-4 bg-slate-900/60 border-t border-slate-800/80 flex items-center gap-2 sm:gap-3">
              <a
                href={`https://wa.me/${flat.whatsappNumber}?text=Hi, I saw your Verified Prime flat listing "${flat.title}" (${flat.hub}) across ${currentCity.name} on Stay&Dine Radar India. What is the earliest check-in date?`}
                target="_blank"
                rel="noreferrer"
      ...
      {filteredFlats.length === 0 && (
        <div className="text-center py-12 glass-card rounded-3xl border border-slate-800 max-w-md mx-auto p-6">
          <Home className="h-10 w-10 text-slate-500 mx-auto mb-3" />
          <h4 className="font-bold text-base text-white">No flats found matching &quot;{searchQuery || filterBhk}&quot;</h4>
          <p className="text-xs text-slate-400 mt-1">
            Try searching across whole city keywords or uncheck the 2 km recommended filter.
          </p>
        </div>
      )}
    </div>
